...
@section('content')
    <div class="container page-content" style="background: none; max-width: none">
        ...
        <div class="list-link-order">
            <a class="{{(!$status) ? 'active' : ''}}" href="{{route('marketplace.vendor.orders.index')}}" >Tất cả</a>
            <a class="{{($status && $status=='processing') ? 'active' : ''}}"
               href="{{route('marketplace.vendor.orders.index',['status'=>'processing'])}}">Chờ xác nhận</a>
            <a class="{{($status && $status=='delivering') ? 'active' : ''}}"
                    href="{{route('marketplace.vendor.orders.index',['status'=>'delivering'])}}">Đang giao</a>
            <a class="{{($status && $status=='delivered') ? 'active' : ''}}"
                    href="{{route('marketplace.vendor.orders.index',['status'=>'delivered'])}}">Đã giao</a>
            <a class="{{($status && $status=='completed') ? 'active' : ''}}"
                    href="{{route('marketplace.vendor.orders.index',['status'=>'completed'])}}">Hoàn thành</a>
            <a class="{{($status && $status=='canceled') ? 'active' : ''}}"
                    href="{{route('marketplace.vendor.orders.index',['status'=>'canceled'])}}">Đã hủy</a>
            <a href="#">Hoàn tiền</a>
        </div>
